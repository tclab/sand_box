...
public class REditarNota extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRowId = savedInstanceState != null ? savedInstanceState.getLong(RNotasDAO.KEY_ROWID) 
                							: null;
		if (mRowId == null) {
			Bundle extras = getIntent().getExtras();            
			mRowId = extras != null ? extras.getLong(RNotasDAO.KEY_ROWID) 
									: null;
		}
        ...
        try {
			if (mRowId != null) {
				...
				mTitleText.setText(note.getString(note
						.getColumnIndexOrThrow(RNotasDAO.KEY_TITLE)));
				mBodyText.setText(note.getString(note
						.getColumnIndexOrThrow(RNotasDAO.KEY_BODY)));
				latitud = Double.parseDouble(note.getString(note
						.getColumnIndexOrThrow(RNotasDAO.KEY_LAT)));
				longitud = Double.parseDouble(note.getString(note
						.getColumnIndexOrThrow(RNotasDAO.KEY_LNG)));
    ...
    protected void onSaveInstanceState(Bundle outState) {
        try {
			...
			outState.putLong(RNotasDAO.KEY_ROWID, mRowId);
		} catch (Exception e) {
		}
    }
    ...
    private void showMap(){
    	try {
			Intent intent = new Intent(this, RMapa.class);
			
			intent.putExtra(RNotasDAO.KEY_LAT, latitud);
			intent.putExtra(RNotasDAO.KEY_LNG, longitud);
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		try {
			...
			case MAPA_EDIT:
				if (extras != null) {
					latitud = extras.getDouble(RNotasDAO.KEY_LAT);
					longitud = extras.getDouble(RNotasDAO.KEY_LNG);
					System.out.println("baba");
				}
				break;
			}
