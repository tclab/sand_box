...
public class PersistenciaGtdDAO {
    ...
    public static void guardarNota(NotaBO nota) throws Exception {
        ...
        dos.writeInt(nota.getSonido().length);
        dos.write(nota.getSonido());
    ...
    public static Vector leerNotas() throws Exception {
        ...
        while (re.hasNextElement()) {
            ...
            int tamSonido = dis.readInt();

            byte[] sonido = new byte[tamSonido];
            if (tamSonido > 0) {
                for (int i = 0; i < tamSonido; i++) {
                    sonido[i] = dis.readByte();
                }
            }
            ...
            nota = new NotaBO(tipoNota,descripcion, lista,sonido);
    ...
    public static void actualizarNota(String nota, int lista) {
        ...
        try {
            ...
            NotaBO guardada = new NotaBO(0,null, 0,null);
            NotaBO aModificar = new NotaBO(0,null, 0,null);
            ...
            guardada = new NotaBO(0,null, 0,null);

            //Se actualizan las notas en el RecordStore
            NotaBO aGuardar = new NotaBO(0,null, 0,null);
            rstore.closeRecordStore();
            ...
            for (int j = 0; j < datosGuardados.size(); j++) {
                ...
                dos.writeInt(aGuardar.getSonido().length);
                dos.write(aGuardar.getSonido());
    ...
    public static void eliminarNota(NotaBO nota) {
        ...
        try {
            ...
            NotaBO guardar = new NotaBO(0, null, 0, null);
            rstore.closeRecordStore();
            rstore.deleteRecordStore("notas");
            rstore = RecordStore.openRecordStore("notas", true);
            for (int j = 0; j < notasGuardadas.size(); j++) {
                ...
                dos.writeInt(guardar.getSonido().length);
                dos.write(guardar.getSonido());
